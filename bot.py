...
'noplaylist': True,
'quiet': True,
'default\_search': 'ytsearch1:',
}

ytdl = yt\_dlp.YoutubeDL(YTDL\_OPTIONS)

# --- Contrôles musique (boutons) ---

class MusicControls(discord.ui.View):
def **init**(self, voice\_client: discord.VoiceClient):
super().**init**(timeout=None)
self.voice\_client = voice\_client
self.looping = False
self.current\_source = None
...
await interaction.response.send\_message("❌ Tu dois être dans un salon vocal pour que je te rejoigne.", ephemeral=True)
return
...
@app\_commands.describe(query="Lien YouTube ou mot-clé")
...
if TOKEN is None:
print("❌ Variable DISCORD\_TOKEN manquante.")
else:
bot.run(TOKEN)
